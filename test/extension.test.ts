...
test('registerHoldExtension registers the "hold" extension', () => {
  ...
  expect(mockHtmx.defineExtension).toHaveBeenCalledWith('hold', expect.any(Object));
...
test('onEvent "htmx:afterProcessNode" correctly sets up event listeners for elements with hx-trigger="hold delay:..."', () => {
  ...
  const mousedownEvent = { type: 'mousedown', preventDefault: mock(() => {}), defaultPrevented: false } as any;
...
test('cancels hold on mouseup before delay', () => {
  ...
  // Start hold
  const mousedownEvent = { type: 'mousedown', preventDefault: mock(() => {}) } as any;
  element.dispatchEvent(mousedownEvent);
  expect(mockTimeouts.length).toBe(1);
...
test('parses different delay values', () => {
  ...
  const mousedownEvent1 = { type: 'mousedown', preventDefault: mock(() => {}) } as any;
  ...
  const mousedownEvent2 = { type: 'mousedown', preventDefault: mock(() => {}) } as any;
...
test('handles touch events', () => {
  ...
  const touchstartEvent = { type: 'touchstart', preventDefault: mock(() => {}) } as any;
