...
function registerHoldExtension() {
  const htmx = (typeof window !== 'undefined' && window.htmx) || (typeof globalThis !== 'undefined' && (globalThis as any).htmx);
  if (!htmx) {
    console.warn("htmx is not available. The 'hold' extension cannot be registered.");
    return;
  }
  ...
  htmx.defineExtension('hold', {
    onEvent: function(name: string, evt: CustomEvent) {
      if (name === 'htmx:afterProcessNode') {
        ...
        if (triggerSpec && triggerSpec.includes('hold')) {
